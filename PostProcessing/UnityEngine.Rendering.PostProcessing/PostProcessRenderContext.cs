    ...
    public sealed class PostProcessRenderContext
    {
...
#else
            cmd.GetTemporaryRT(nameID, desc.width, desc.height, desc.depthBufferBits, filter, desc.colorFormat, readWrite, desc.msaaSamples, desc.enableRandomWrite);
#endif
        }
                                                        ...
                                                        RenderTextureReadWrite readWrite = RenderTextureReadWrite.Default, int widthOverride = 0, int heightOverride = 0)
        {
            ...
            return RenderTexture.GetTemporary(
                width: desc.width,
                height: desc.height,
                depthBuffer: desc.depthBufferBits,
                format: desc.colorFormat,
                readWrite: desc.readWrite,
