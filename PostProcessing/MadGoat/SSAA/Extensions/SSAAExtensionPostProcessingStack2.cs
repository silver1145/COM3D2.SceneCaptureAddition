...
namespace MadGoat.SSAA {
    ...
    public class SSAAExtensionPostProcessingStack2 : SSAAExtensionBase {
        ...
        public override void OnUpdate(MadGoatSSAA ssaaInstance) {
            ...
            // workaround for issues with pp stack on losing editor focus
            if (ppLayerCurrent && ppLayerCurrent.enabled && hasInitialized) ppLayerCurrent.enabled = false;
            else if (ppLayerCurrent && !hasInitialized) ppLayerCurrent.enabled = true;
