...
using System.IO;
using COM3D2.PostProcessing.Plugin.Utils;
...
#pragma warning restore CS0414
        ...
        private void Update() {
            ...
            // avoid issues with camera enabling/disabling
            if (RenderCamera.enabled != currentCamera.enabled) renderCamera.enabled = currentCamera.enabled;
        ...
        protected virtual void OnInitialize() {
            ...
            if (childCams.Find(x => x.name.Contains("_SSAA")) == null) {
            ...
            // Grab existing camera
                ...
                renderCamera = childCams.Find(x => x.name.Contains("_SSAA"));
        ...
        protected virtual void OnInitializeProps() {
            ...
            var shaders = AssetLoader.ssaaDef;
            ...
            ShaderBilinear = AssetLoader.ssaaBilinear;
            ShaderBicubic = AssetLoader.ssaaBicubic;
            ShaderDefault = AssetLoader.ssaaDef;
            ...
            if (ShaderFXAA == null) ShaderFXAA = AssetLoader.ssaaFSS;
            if (ShaderAlpha == null) ShaderAlpha = AssetLoader.ssaaAlpha;
        ...
        private void UpdateRtSizes() {
            ...
            // Make sure we dont have 0x0 render targets (thanks to hdrp..)
            if (CameraTargetWidth * InternalRenderMultiplier < 128 && CameraTargetHeight * (InternalRenderMode == RenderMode.PerAxisScale ? InternalRenderMultiplierVertical : InternalRenderMultiplier) < 128)
                return;
...
#if SSAA_HDRP || SSAA_URP
                // fix for flickering on resolution change
                // Apply old image to rescaled RT
        ...
        private void HandleScreenshot() {
            ...
            Material screenshotMat = new Material(AssetLoader.ssaaBilinear);
...
#if SSAA_URP && UNITY_2019_3 // has been removed again from 2020 onwards
